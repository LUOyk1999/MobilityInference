...
class smot(object):
    ...
    def _retrieve_neighbors(self, index_center, matrix):
        ...
        while (start >= 0):
            if (distance(int(matrix[start][2]), int(matrix[start][3]), int(center_point[2]),
                         int(center_point[3])) <= self.spatial_threshold):
                start -= 1
            else:
                break
        while (end < len(matrix)):
            if (distance(int(matrix[end][2]), int(matrix[end][3]), int(center_point[2]),
                         int(center_point[3])) <= self.spatial_threshold):
                end += 1
            else:
                break
        neigborhood = []
        if (matrix[end-1][1] - matrix[start+1][1] >= self.temporal_threshold):
            neigborhood = matrix[start+1:end, 5].tolist()
...
Smot = smot(spatial_threshold=800, temporal_threshold=30 * 60)
...
print(
    'resample_rate:%f\nvp:%f\nvr:%f\nsp:%f\nsr:%f\nacc:%f\nv_f1:%f\ns_f1:%f\nacc_f1:%f\nvn_p:%d\nsn_p:%d\nvn:%d\nsn:%d\ntime:%f' % (
        float(args.resample_rate), vp, vr, sp, sr, acc, v_f1, s_f1, acc_f1, vn_p, sn_p, vn, sn,
        time.time() - start_time))

result_filename = file_dir + "perf_smot_" + testfile_name.replace('-' + format(float(args.resample_rate), '.1f'),
                                                                    '') + '.csv'
